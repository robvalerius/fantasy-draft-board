...
/* Both boards are served from the same github.io origin, and localStorage is
   scoped to the origin rather than the path - so without a per-league key the
   Sleeper board would read the Yahoo draft's picks and vice versa. The original
   league keeps its original key so an in-progress draft is never orphaned. */
const KEY = DATA.league_slug ? `ff-draft-${DATA.league_slug}` : 'bftb14-draft';
